...
export class DobavljacService {
 ...
 constructor(private httpClient: HttpClient) { }
  ...
  public createDobavljac(dobavljac:Dobavljac):Observable<any>{
    return this.httpClient.post("http://localhost:8080/dobavljacs", dobavljac);
  }

  public updateDobavljac(dobavljac:Dobavljac):Observable<any>{
    return this.httpClient.put(`http://localhost:8080/dobavljacs?id=${dobavljac.id}`, dobavljac);
  }

  public deleteDobavljac(id:number):Observable<any>{
    return this.httpClient.delete(`http://localhost:8080/dobavljacs?id=${id}`);
  }
}